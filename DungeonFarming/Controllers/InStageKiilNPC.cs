...
public class KillNPC : ControllerBase
{
    ...
    public async Task<KillNPCResponse> Post(KillNPCRequest request)
    {
        ...
        List<int> currKilledNpcs = new List<int>();
        currKilledNpcs = await _redisDb.GetKilledNPCList(request.UID);
        ...
        var errorCode = await _redisDb.PlayerKillNPC(request.UID, request.NPCCode);
